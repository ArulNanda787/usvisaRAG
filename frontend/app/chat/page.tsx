...
const s = {
  ...
  headerName: { fontSize: 13, fontWeight: 600, color: "#fff", lineHeight: 1 } as CSSProperties,
  headerSub: { fontSize: 11, color: "var(--muted)", marginTop: 2 } as CSSProperties,
  headerRight: { display: "flex", alignItems: "center", gap: 12 } as CSSProperties,
  backBtn: { fontSize: 12, color: "var(--muted)", cursor: "pointer", background: "none", border: "1px solid var(--border)", borderRadius: 8, padding: "5px 10px", fontFamily: "'DM Sans', sans-serif", transition: "color 0.15s, border-color 0.15s" } as CSSProperties,
  ...
  statusLabel: { fontSize: 11, color: "var(--muted)" } as CSSProperties,
  ...
  emptyTitle: { fontSize: 26, fontWeight: 600, color: "#fff", marginBottom: 4, textAlign: "center" } as CSSProperties,
  emptySub: { fontSize: 13, color: "var(--muted)", textAlign: "center" } as CSSProperties,
  suggestLabel: { fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--muted)", textAlign: "center", marginBottom: 10 } as CSSProperties,
  suggestGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, width: "100%", maxWidth: 480 } as CSSProperties,
  suggestBtn: { display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface)", cursor: "pointer", textAlign: "left", transition: "border-color 0.15s, background 0.15s" } as CSSProperties,
  suggestIcon: { fontSize: 16, flexShrink: 0 } as CSSProperties,
  suggestText: { fontSize: 12, color: "var(--muted2)", lineHeight: 1.3 } as CSSProperties,
  ...
  bubbleUser: { maxWidth: "72%", padding: "10px 16px", borderRadius: "18px 18px 4px 18px", background: "linear-gradient(135deg, #1e3a6e 0%, #2d5be3 100%)", color: "#fff", fontSize: 13, lineHeight: 1.6 } as CSSProperties,
  bubbleAssistant: { maxWidth: "80%", padding: "12px 16px", borderRadius: "18px 18px 18px 4px", border: "1px solid var(--border2)", background: "var(--surface)", fontSize: 13, lineHeight: 1.7 } as CSSProperties,
  ...
  srcLabel: { fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 6 } as CSSProperties,
  ...
  input: { flex: 1, background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 12, padding: "10px 16px", fontSize: 13, color: "var(--text)", outline: "none", fontFamily: "'DM Sans', sans-serif", transition: "border-color 0.15s" } as CSSProperties,
  clearBtn: { width: 34, height: 34, borderRadius: 10, flexShrink: 0, border: "1px solid var(--border)", background: "transparent", color: "var(--muted)", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", transition: "border-color 0.15s, color 0.15s" } as CSSProperties,
  sendBtn: { width: 36, height: 36, borderRadius: 10, flexShrink: 0, border: "none", background: "linear-gradient(135deg, #1e3a6e, #2d5be3)", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "opacity 0.15s" } as CSSProperties,
  footer: { fontSize: 10, color: "var(--muted)", textAlign: "center", marginTop: 6 } as CSSProperties,
};

function CategoryTag({ cat }: { cat: string }) {
  const color = CATEGORY_COLORS[cat] ?? "#64748b";
  return (
    <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", padding: "2px 8px", borderRadius: 20, border: `1px solid ${color}50`, background: `${color}18`, color }}>
      {cat.replace(/_/g, " ")}
    </span>
  );
}
...
function SourcePill({ url }: { url: string }) {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, padding: "3px 10px", borderRadius: 20, border: "1px solid var(--border2)", color: "var(--accent)", background: "var(--accent-dim)", textDecoration: "none" }}>
      {domain}
    </a>
  );
}
      ...
      <header style={s.header}>
        ...
        <div style={s.headerRight}>
          <button style={s.backBtn} onClick={() => router.push("/")}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "#fff"; (e.currentTarget as HTMLElement).style.borderColor = "var(--border2)"; }}
            ...
            <div style={s.emptyWrap}>
              ...
              <div style={{ width: "100%", maxWidth: 480 }}>
                ...
                <div style={s.suggestGrid}>
                  {SUGGESTED.map(s2 => (
                    <button key={s2.label} style={s.suggestBtn} onClick={() => sendQuery(s2.query)}
